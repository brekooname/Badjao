    ...
    public class ExcelQuery : Query
    {
        ...
        public void WriteExcelFile( DataTable table, string filePath )
        {
                ...
                && !string.IsNullOrEmpty( filePath ) )
            {
                try
                {
                    using ExcelPackage _excelPackage = ExcelQuery.ReadExcelFile( filePath );
        ...
        private void ExportToDataGrid( DataGridView dataGrid )
        {
            try
            {
                ...
                ExcelPackage _application = new ExcelPackage( _stream );
                ExcelWorkbook _workbook = _application.Workbook;
                ExcelWorksheet _worksheet = _workbook.Worksheets[ 1 ];
                ExcelRange _range = _worksheet.SelectedRange;
                int _rows = _range.Rows;
                int _columns = _range.Columns;
