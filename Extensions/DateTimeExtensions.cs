...
namespace BudgetExecution
{
    ...
    public static class DateTimeExtensions
    {
        ...
        public static DateTime HoursAgo( this int hours )
        {
            try
            {
                TimeSpan _timeSpan = new TimeSpan( hours, 0, 0 );
                return DateTime.Now.Subtract( _timeSpan );
            }
            catch( Exception ex )
            {
        ...
        public static DateTime MinutesAgo( this int minutes )
        {
            try
            {
                TimeSpan _timeSpan = new TimeSpan( 0, minutes, 0 );
                return DateTime.Now.Subtract( _timeSpan );
            }
            catch( Exception ex )
            {
        ...
        public static DateTime SecondsAgo( this int seconds )
        {
            try
            {
                TimeSpan _timeSpan = new TimeSpan( 0, 0, seconds );
                return DateTime.Now.Subtract( _timeSpan );
            }
            catch( Exception ex )
            {
        ...
        public static int CountWeekDays( this DateTime startDate, DateTime endDate )
        {
            try
            {
                TimeSpan _timeSpan = endDate - startDate;
                Console.WriteLine( _timeSpan.Days );
                int _days = 0;

                for( int i = 0; i < _timeSpan.Days; i++ )
                {
                    DateTime _dateTime = startDate.AddDays( i );
        ...
        public static TimeSpan DateDelta( this DateTime startDate, DateTime endDate )
        {
            try
            {
                TimeSpan _timeSpan = startDate.Subtract( endDate );
                return _timeSpan;
            }
            catch( Exception ex )
            {
